...
void options_keybinds_state::init(font_sprite_painter* font_painter_, GLuint vbo, GLuint vao)
{
	...
	footer_height = font_painter->get_lineskip() + font_padding + element_padding * 2;
...
OPTIONS_KEYBINDS_RESULT options_keybinds_state::input(SDL_Event& e)
{
	if(e.type == SDL_WINDOWEVENT)
	{
		...
		case SDL_WINDOWEVENT_HIDDEN:
			...
			if(requested_button != NULL)
			{
				...
				requested_button->text = keybind.cvar_write();
	...
	// footer buttons
	{
		...
		case BUTTON_RESULT::TRIGGER:
			// trigger will eat
			// slog("revert click\n");
            // TODO: a list is absolutely not needed, unless I had a undo/redo button...
			for(auto rit = history.rbegin(); rit != history.rend(); ++rit)
			{
				rit->slot.keybind.key_binds = rit->value;
				rit->slot.text = rit->slot.keybind.cvar_write();
			}
			clear_history();
			return OPTIONS_KEYBINDS_RESULT::CONTINUE;
		...
		case BUTTON_RESULT::TRIGGER:
			...
			for(auto& button : buttons)
			{
				...
				button.text = button.keybind.cvar_write();
			}
			revert_button.set_disabled(false);
			return OPTIONS_KEYBINDS_RESULT::CONTINUE;
		case BUTTON_RESULT::ERROR: return OPTIONS_KEYBINDS_RESULT::ERROR;
	...
	*/
	...
	// draw the keybind description text
	{
		...
		for(auto& button : buttons)
		{
            ...
            if(!button.button.draw_buffer(button.text.c_str(), button.text.size()))
            {
                return false;
            }
		}
...
void options_keybinds_state::resize_view()
{
	...
	float max_width = 400 * (font_painter->get_lineskip() / 16.f);
